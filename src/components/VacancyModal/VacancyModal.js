	...
	return (
		<React.Fragment>
			<Modal show={props.open} onHide={props.handleClose} centered size={"lg"}>
				...
				<Modal.Body>
					<span className="d-inline-block mb-2 font-weight-light">{props.data.company.name}</span>
					<p>
						...
						{props.data.term ?
							<React.Fragment><br/> <span className="mr-2">Бонусы:</span><span>{props.data.term.text}</span></React.Fragment>
						: null}
					</p>
					<hr/>
					<p>
					...
					<p>
						...
						{props.data.requirement.experience === 0 ? <span>Без опыта</span> : <span>{props.data.requirement.experience} года</span>}
					...
					<p>
						...
						{props.data.company.phone ? <a href="#">{props.data.company.phone}</a> : <span>Не указан</span>}
						<br/>
						<span className="mr-2">E-mail:</span>
						{props.data.company.email ? <a href="#">{props.data.company.email}</a> : <span>Не указан</span>}
						<br/>
